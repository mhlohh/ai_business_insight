...
# Page config
st.set_page_config(
    page_title="AI Product Insights Dashboard",
    page_icon="📈",
    layout="wide"
)
...
st.sidebar.markdown(f'<div class="neumorphic-card" style="padding: 15px;">'
...
# Main Layout Header
st.markdown(f'<div class="main-title">{SVG_SEARCH}AI Product Review Insights</div>', unsafe_allow_html=True)
...
if error:
    ...
    # Render Insights
    insights = insights_data.get("insights", []) if isinstance(insights_data, dict) else insights_data
    ...
    st.sidebar.markdown(f'<div class="neumorphic-card" style="padding: 12px; margin-top: 20px;">'
    ...
    if selected_categories:
        df_filtered = df_filtered[df_filtered['category'].str.title().isin(selected_categories)]
    else:
    ...
    avg_conf = df_filtered['confidence'].mean() * 100 if 'confidence' in df_filtered.columns and not df_filtered.empty else 0
    # Top Category
    top_cat = df_filtered['category'].mode()[0] if 'category' in df_filtered.columns and not df_filtered['category'].empty else "N/A"
    ...
    if not df_filtered.empty:
        ...
        with col1:
            st.markdown(f'<div class="neumorphic-card">'
                        f'<h3 style="margin-top:0; display: flex; align-items: center;">'
                        f'{SVG_CHART}Insight Categories Distribution</h3>', unsafe_allow_html=True)
            ...
            bars = ax.bar(category_counts.index, category_counts.values, color='#4a6b82', edgecolor='#354e60', width=0.6, linewidth=1.5)
        ...
        with col2:
            st.markdown(f'<div class="neumorphic-card">'
                        f'<h3 style="margin-top:0; display: flex; align-items: center;">'
                        f'{SVG_TRENDING}Insights ranked by Score</h3>', unsafe_allow_html=True)
            
            if 'score' in df_filtered.columns:
                df_sorted = df_filtered.sort_values(by='score', ascending=True).tail(5) # Top 5
                fig, ax = plt.subplots(figsize=(6, 4.5), dpi=200)
                fig.patch.set_facecolor('#e0e5ec')
                ax.set_facecolor('#e0e5ec')
                
                short_labels = [label[:25] + "..." if len(label) > 25 else label for label in df_sorted['insight']]
                ax.barh(short_labels, df_sorted['score'], color='#b27a50', edgecolor='#8c5e3d', height=0.5, linewidth=1.5)
    ...
    st.markdown(f'<h2 style="margin-top:20px; color:#4a6b82; display: flex; align-items: center;">'
                f'{SVG_LIST}AI Insights List</h2>', unsafe_allow_html=True)
