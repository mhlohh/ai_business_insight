...
from app.database import get_cached_analysis, cache_analysis, clear_cache, get_reviews, get_product
...
async def analyze_product_reviews(product_id: int):
    ...
    # Check Cache
    cached_analysis = get_cached_analysis(product_id)
    ...
    start_time = time.time()  # We need time module
    analysis_result = await ask(reviews_prompt)
    elapsed_time = time.time() - start_time

    # Store in cache
    cache_analysis(product_id, analysis_result)
        ...
        try:
            async for event in ask_stream(reviews_prompt):
                if event["status"] == "completed":
                    ...
                    event["cached"] = False
                    # Store in cache
                    cache_analysis(product_id, event["result"])
